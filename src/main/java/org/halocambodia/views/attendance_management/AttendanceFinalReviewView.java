...
import org.halocambodia.services.EmployeeAttendanceFinalReviewService;
...
@Route(value = "attendance-final-review", layout = MainLayout.class)
@PageTitle("HR Attendance Final Review | ការពិនិត្យវត្តមានចុងក្រោយដោយ HR")
@PermitAll
public class AttendanceFinalReviewView extends VerticalLayout implements BeforeEnterObserver {
    ...
    private static volatile List<LegendDef> LEGENDS = List.of(
            ...
            new LegendDef("!", "Pending HR Review | រង់ចាំ HR", "#DB2777"));

    private final EmployeeAttendanceFinalReviewService service;
    ...
    private final Checkbox pendingOnly =
            new Checkbox("Pending HR Only | រង់ចាំ HR តែប៉ុណ្ណោះ");
    ...
    public AttendanceFinalReviewView(EmployeeAttendanceFinalReviewService service,
    ...
    private void configurePage() {
        ...
        Button verifyButton = new Button("Final Verify Selected | ផ្ទៀងផ្ទាត់ចុងក្រោយ",
                VaadinIcon.CHECK_CIRCLE.create(), e -> confirmVerification());
        verifyButton.addThemeVariants(ButtonVariant.LUMO_PRIMARY);
        verifyButton.setEnabled(authenticatedUser.hasPage(AttendanceFinalReviewView.class, AccessPageType.UPDATED_PAGE));
    ...
    private void configureGrid(LocalDate start, LocalDate end) {
        ...
        Grid.Column<EmployeeRow> pendingColumn = grid.addColumn(EmployeeRow::pendingCount)
                .setHeader("Pending HR | រង់ចាំ HR")
        ...
        Grid.Column<EmployeeRow> verifiedColumn = grid.addColumn(EmployeeRow::verifiedCount)
                .setHeader("Final Verified | បានផ្ទៀងផ្ទាត់ចុងក្រោយ")
        ...
        if (!monthColumns.isEmpty()) {
            ...
            monthHeader.join(pendingColumn, verifiedColumn)
                    .setComponent(hrSummaryHeader());
        }
    }

    private static Div hrSummaryHeader() {
        Div header = new Div();
        header.setText("HR Review Summary | សង្ខេបការពិនិត្យ HR");
        header.getStyle()
                .set("font-weight", "700")
    ...
    private Span dayBadge(EmployeeRow row, LocalDate date) {
        ...
        if (cell != null) {
            ...
            badge.addClickListener(event -> openDayHrDialog(row, date, cell));
        }
        return badge;
    }

    private void openDayHrDialog(EmployeeRow row, LocalDate date, DayCell cell) {
        CustomDialog dialog = new CustomDialog(
                "Daily HR Final Review | ការពិនិត្យចុងក្រោយដោយ HR ប្រចាំថ្ងៃ");
        ...
        VerticalLayout content = new VerticalLayout(
                ...
                new Span("QC Verified By | ផ្ទៀងផ្ទាត់ដោយ QC: "
                        + (record.getQcByEmployee() == null ? ""
                                : safe(record.getQcByEmployee().getNameEn()))),
                new Span("QC Date | កាលបរិច្ឆេទ QC: "
                        + (record.getQcDate() == null ? "" : record.getQcDate())),
                new Span("Remark | កំណត់សម្គាល់: " + safe(record.getRemark())));
        content.setPadding(false);
        dialog.add(content);

        Button verify = new Button("Final Verify Day | ផ្ទៀងផ្ទាត់ចុងក្រោយ", event -> {
            try {
                EmployeeAttendanceFinalReviewService.VerifyResult result =
                        service.verifyDay(record.getId());
                if (result.verified() == 0) {
                    showError(result.skipped() > 0
                            ? "This day has a duplicate or is not eligible for HR final review."
                            : "This day cannot be final verified.");
                    return;
                }
                dialog.close();
                refresh();
                Notification.show("HR final review completed | HR បានពិនិត្យចុងក្រោយ", 2500,
        ...
        Button returnButton = new Button("Return to QC | បញ្ជូនត្រឡប់ទៅ QC",
    ...
    private void openReturnDialog(CustomDialog parent, DayCell cell) {
        CustomDialog dialog = new CustomDialog(
                "Return to QC | បញ្ជូនត្រឡប់ទៅ QC");
        ...
        Button confirm = new Button("Return | បញ្ជូនត្រឡប់", event -> {
            try {
                ...
                dialog.close();
                refresh();
                Notification.show(returned + " record(s) returned to QC review.",
    ...
    private void applyFilter() {
        ...
        summary.setText(rows.size() + " employee(s) | បុគ្គលិក · "
                ...
                + " pending HR review | រង់ចាំ HR · "
                + rows.stream().mapToLong(EmployeeRow::verifiedCount).sum()
                + " final verified | បានផ្ទៀងផ្ទាត់ចុងក្រោយ");
    }
    ...
    private void confirmVerification() {
        ...
        if (ids.isEmpty()) {
            showError("The selected employees have no attendance pending HR final review. "
                    + "| បុគ្គលិកដែលបានជ្រើសមិនមានវត្តមានរង់ចាំ HR ពិនិត្យចុងក្រោយទេ។");
            return;
        }
        CustomDialog dialog = new CustomDialog(
                "Confirm HR Final Review | បញ្ជាក់ការពិនិត្យចុងក្រោយដោយ HR");
        dialog.setWidth("min(560px, 95vw)");

        Paragraph confirmation = new Paragraph(
                "Final verify " + ids.size() + " attendance record(s) for " + selected.size()
                        + " employee(s)?\n\nផ្ទៀងផ្ទាត់ចុងក្រោយកំណត់ត្រាវត្តមាន " + ids.size()
        ...
        Button confirm = new Button("Final Verify | ផ្ទៀងផ្ទាត់ចុងក្រោយ", e -> {
            try {
                EmployeeAttendanceFinalReviewService.VerifyResult result = service.verify(ids);
                dialog.close();
    ...
    private static List<LegendDef> loadLegends(LeaveTypeRepository repository) {
        ...
        definitions.put("!", new LegendDef("!", "Pending HR Review | រង់ចាំ HR", "#DB2777"));
    ...
    private static final class EmployeeRowBuilder {
        ...
        private EmployeeRow build() {
            ...
            for (Map.Entry<LocalDate, List<EmployeeAttendanceDetail>> entry : byDay.entrySet()) {
                ...
                List<Long> pending = records.stream().filter(r -> r.getHrVerificationBy() == null)
                ...
                days.put(entry.getKey(), new DayCell(dayCode, dayVerified, records.size() > 1,
                        ...
                        + (dayVerified ? "HR Final Verified | HR បានផ្ទៀងផ្ទាត់ចុងក្រោយ"
                                : "Pending HR Final Review | រង់ចាំ HR ពិនិត្យចុងក្រោយ")
