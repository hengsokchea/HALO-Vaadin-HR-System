...
import org.halocambodia.services.DisabilityService;
...
@Route(value = "disability-types", layout = MainLayout.class)
@PageTitle("Disability Types | ប្រភេទពិការភាព")
@PermitAll
public class DisabilityTypeView extends PageDialogLayout<DisabilityType, DisabilityService> {

    private final Optional<User> currentUserLogin;
    private final PasswordEncoder passwordEncoder;

    // ===================== Editor fields =====================
    private final TextField disabilityTypeName = new TextField("Disability Type | ប្រភេទពិការភាព");
    private final TextArea note = new TextArea("Note | កំណត់សម្គាល់");

    public DisabilityTypeView(DisabilityService service,
                              UserService userService,
                              AuthenticatedUser authenticatedUser,
                              PasswordEncoder passwordEncoder) {
        super(DisabilityType.class, service, userService, authenticatedUser);
    ...
    private void preload() {
        disabilityTypeName.setWidthFull();
        disabilityTypeName.setPlaceholder("Enter disability type... | បញ្ចូលប្រភេទពិការភាព...");
        disabilityTypeName.setClearButtonVisible(true);
    ...
    protected void configureEditorLayout() throws Exception {
        ...
        FormLayout form = new FormLayout(disabilityTypeName, note);
        form.setWidthFull();
    ...
    @Override
    protected void populateForm(DisabilityType entity) throws Exception {
        this.entity = entity;
    ...
    @Override
    protected DisabilityType createNewEntity() {
        return new DisabilityType();
    }

    @Override
    protected void focusFirstField() {
        disabilityTypeName.focus();
    }

    @Override
    protected void binderField() {

        binder.forField(disabilityTypeName)
                .asRequired("Disability Type is required | ត្រូវការប្រភេទពិការភាព")
                .bind(DisabilityType::getDisabilityTypeName, DisabilityType::setDisabilityTypeName);

        binder.bind(note, DisabilityType::getNote, DisabilityType::setNote);
    }

    // ===================== Grid columns =====================

    @Override
    protected List<ColumnDef<DisabilityType>> getColumnDefs() {
        return List.of(
                col("id", "ID",
                        DisabilityType::getId,
                        e -> e.getId() == null ? "" : e.getId().toString()),

                col("disabilityTypeName", disabilityTypeName.getLabel(),
                        DisabilityType::getDisabilityTypeName,
                        e -> e.getDisabilityTypeName() == null ? "" : e.getDisabilityTypeName()),

                col("note", note.getLabel(),
                        DisabilityType::getNote,
    ...
    @Override
    protected Specification<DisabilityType> buildCombinedSpecification() {
        return (root, query, cb) -> {
            try {
                List<Predicate> predicates = new ArrayList<>();
                List<String> sqlFilter = new ArrayList<>();

                Join<DisabilityType, User> userCreated = root.join("userCreated", JoinType.LEFT);
                Join<DisabilityType, User> userUpdated = root.join("userUpdated", JoinType.LEFT);
                ...
                if (quick != null && !quick.isBlank()) {
                    ...
                    predicates.add(cb.or(
                            ...
                            buildLikePredicate(cb, cb.lower(root.get("disabilityTypeName")), like),
                ...
                if (advPanel != null) {
                    ...
                    TextField nameField = advPanel.getField("disabilityTypeName", TextField.class);
                    advanceFilterBuildLikePredicate(cb, root.get("disabilityTypeName"),
                            nameField == null ? null : nameField.getValue(),
                            disabilityTypeName.getLabel(), predicates, sqlFilter);
