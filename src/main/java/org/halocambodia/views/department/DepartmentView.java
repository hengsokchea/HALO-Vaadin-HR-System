package org.halocambodia.views.department;
...
import org.halocambodia.services.DepartmentService;
...
@PageTitle("Departments | ផ្នែក/នាយកដ្ឋាន")
@PermitAll
public class DepartmentView extends PageDialogLayout<Department, DepartmentService> {

    private final Optional<User> currentUserLogin;
    private final PasswordEncoder passwordEncoder;

    // ===================== Editor fields =====================
    private final TextField name = new TextField("Department (En) | ផ្នែក/នាយកដ្ឋាន (អង់គ្លេស)");
    private final TextField nameKH = new TextField("Department (Kh) | ផ្នែក/នាយកដ្ឋាន (ខ្មែរ)");
    ...
    public DepartmentView(DepartmentService service,
        ...
        super(Department.class, service, userService, authenticatedUser);
    ...
    private void preload() {
        name.setWidthFull();
        name.setPlaceholder("Enter department... | បញ្ចូលផ្នែក/នាយកដ្ឋាន...");
        name.setClearButtonVisible(true);

        nameKH.setWidthFull();
        nameKH.setPlaceholder("Enter Khmer department... | បញ្ចូលផ្នែក/នាយកដ្ឋានខ្មែរ...");
        nameKH.setClearButtonVisible(true);

        remarks.setWidthFull();
        remarks.setPlaceholder("Optional... | ជាជម្រើស...");
        remarks.setClearButtonVisible(true);
        remarks.setMinHeight("110px");
    }
    ...
    protected void configureEditorLayout() throws Exception {
        ...
        FormLayout form = new FormLayout(name, nameKH, remarks);
        form.setWidthFull();
        ...
        // Make remarks span 2 columns on wide layout
    ...
    @Override
    protected void populateForm(Department entity) throws Exception {
        this.entity = entity;
    ...
    protected void binderField() {
        ...
        binder.forField(name)
                .asRequired("Department EN is required | ត្រូវការឈ្មោះផ្នែក(EN)")
                .bind(Department::getName, Department::setName);

        binder.forField(nameKH)
                .asRequired("Department KH is required | ត្រូវការឈ្មោះផ្នែក(KH)")
                .bind(Department::getNameKH, Department::setNameKH);

        binder.bind(remarks, Department::getRemarks, Department::setRemarks);
    }

    // ===================== Grid columns =====================

    @Override
    protected List<ColumnDef<Department>> getColumnDefs() {
        return List.of(
                col("id", "ID",
                        Department::getId,
                        e -> e.getId() == null ? "" : e.getId().toString()),

                col("name", name.getLabel(),
                        Department::getName,
                        e -> e.getName() == null ? "" : e.getName()),

                col("nameKH", nameKH.getLabel(),
                        Department::getNameKH,
                        e -> e.getNameKH() == null ? "" : e.getNameKH()),

                col("remarks", remarks.getLabel(),
                        Department::getRemarks,
                        e -> e.getRemarks() == null ? "" : e.getRemarks()),
    ...
    @Override
    protected Specification<Department> buildCombinedSpecification() {
        return (root, query, cb) -> {
            try {
                List<Predicate> predicates = new ArrayList<>();
                List<String> sqlFilter = new ArrayList<>();

                Join<Department, User> userCreated = root.join("userCreated", JoinType.LEFT);
                Join<Department, User> userUpdated = root.join("userUpdated", JoinType.LEFT);
                ...
                if (quick != null && !quick.isBlank()) {
                    ...
                    predicates.add(cb.or(
                            ...
                            buildLikePredicate(cb, cb.lower(root.get("name")), like),
                            buildLikePredicate(cb, cb.lower(root.get("nameKH")), like),
                            buildLikePredicate(cb, cb.lower(root.get("remarks")), like),
                ...
                if (advPanel != null) {
                    ...
                    TextField nameField = advPanel.getField("name", TextField.class);
                    advanceFilterBuildLikePredicate(cb, root.get("name"),
                            nameField == null ? null : nameField.getValue(),
                            name.getLabel(), predicates, sqlFilter);

                    TextField nameKhField = advPanel.getField("nameKH", TextField.class);
                    advanceFilterBuildLikePredicate(cb, root.get("nameKH"),
                            nameKhField == null ? null : nameKhField.getValue(),
                            nameKH.getLabel(), predicates, sqlFilter);

                    TextField remarksField = advPanel.getField("remarks", TextField.class);
                    advanceFilterBuildLikePredicate(cb, root.get("remarks"),
                            remarksField == null ? null : remarksField.getValue(),
                            remarks.getLabel(), predicates, sqlFilter);
