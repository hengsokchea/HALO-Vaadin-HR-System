...
@Service
public class EmployeeLeaveRequestService implements GenericService<EmployeeLeave> {
	  ...
	  public EmployeeLeaveRequestService(EmployeeLeaveRepository repository,AuthenticatedUser authenticatedUser,AttachmentRepository attachmentRepository) {
	  ...
	  public Optional<EmployeeLeave> findById(Long id) {
	  ...
	  public void delete(Set<EmployeeLeave> entities) {
	      if (!authenticatedUser.hasPage(LeaveRequestView.class, AccessPageType.DELETED_PAGE)) {
	  ...
	  public EmployeeLeave update(EmployeeLeave incoming) {
	      ...
	      if (!authenticatedUser.hasPage(LeaveRequestView.class, accessPageType)) {
		...
		@Transactional
		public List<EmployeeLeave> saveAll(List<EmployeeLeave> entities) {
			...
			// Validate permissions
			if (!authenticatedUser.hasPage(LeaveRequestView.class, AccessPageType.UPDATED_PAGE)) {
				throw new IllegalArgumentException("You don't have permission to perform this operation");
			}

			List<EmployeeLeave> managedEntities = new java.util.ArrayList<>();

			for (EmployeeLeave incoming : entities) {
				boolean isNew = incoming.getId() == null;
				EmployeeLeave managed = isNew ? new EmployeeLeave() : repository.findById(incoming.getId())
						.orElseThrow(() -> new IllegalArgumentException("Leave request with ID " + incoming.getId() + " not found"));
			...
			// Save all entities
			List<EmployeeLeave> savedEntities = new java.util.ArrayList<>();
			for (EmployeeLeave managed : managedEntities) {
				savedEntities.add(entityManager.merge(managed));
			}
			
			// Flush to ensure all changes are written to the database
			entityManager.flush();
