...
@Service
public class EmployeeAttendanceFinalReviewService {
    ...
    public EmployeeAttendanceFinalReviewService(EmployeeAttendanceDetailRepository repository,
    ...
    public List<EmployeeAttendanceDetail> findForVerification(LocalDate start, LocalDate end) {
        ...
        List<EmployeeAttendanceDetail> records =
                repository.findForHrFinalReview(
    ...
    public int returnDay(Collection<Long> ids, String reason) {
        ...
        for (EmployeeAttendanceDetail record : records) {
            if (!hasBranchAccess(record, allowedBranchIds)
                    || record.getEntryStatus() != AttendanceEntryStatus.SUBMITTED
                    || record.getQcStatus() != AttendanceQcStatus.VERIFIED
                    || record.getQcByEmployee() == null
                    || record.getHrVerificationBy() != null) continue;
    ...
    private boolean eligible(EmployeeAttendanceDetail record) {
        return record.getEntryStatus() == AttendanceEntryStatus.SUBMITTED
                && record.getQcStatus() == AttendanceQcStatus.VERIFIED
                && record.getQcByEmployee() != null
                && record.getHrVerificationBy() == null;
    }

    private void applyVerified(EmployeeAttendanceDetail record, Employee verifier, User user) {
        record.setHrVerificationBy(verifier);
        record.setHrVerificationPosition(verifier.getPositions());
        record.setHrVerificationDate(LocalDate.now(DateTimeUtilFormart.CAMBODIA_ZONE));
    ...
    private void requirePermission(AccessPageType type) {
        if (!authenticatedUser.hasPage(AttendanceFinalReviewView.class, type))
            throw new IllegalArgumentException("You do not have permission for HR final review.");
    }
