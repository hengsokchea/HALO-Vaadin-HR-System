...
@Service
public class GoalSettingService implements GenericService<GoalSetting> {
	  ...
	  public GoalSettingService(GoalSettingRepository repository,AuthenticatedUser authenticatedUser,GoalSettingDetailRepository goalSettingDetailRepository) {
	  ...
	  public void delete(Set <GoalSetting> entity) {
		  if (!authenticatedUser.hasPage(GoalSupervisorView.class, AccessPageType.DELETED_PAGE)) {
	  ...
	  public GoalSetting update(GoalSetting entityValue) {
	      ...
	      if (!authenticatedUser.hasPage(GoalSupervisorView.class, accessPageType)) {
	    ...
	    public void deleteGoalSettingDetail(Long detailId) {
			  if (!authenticatedUser.hasPage(GoalSettingEntryView.class, AccessPageType.DELETED_PAGE)) {
				  throw new IllegalArgumentException("You don't have permission to do this operation ");
		       }
