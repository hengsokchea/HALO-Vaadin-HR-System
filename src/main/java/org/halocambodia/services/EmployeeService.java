...
public class EmployeeService implements GenericService<Employee> {
	  ...
	  public EmployeeService(EmployeeRepository repository,AuthenticatedUser authenticatedUser) {
	  ...
	  public void delete(Set <Employee> entity) {
		  if (!authenticatedUser.hasPage(NationalStaffActiveView.class, AccessPageType.DELETED_PAGE)) {
	  ...
	  public Employee update(Employee entityValue) {
	      ...
	      if (!authenticatedUser.hasPage(NationalStaffActiveView.class, accessPageType)) {
