...
@Service
public class DataExplorerService implements GenericService<DataExplorer> {
	
	
	  private final DataExplorerRepository repository;
	  private final AuthenticatedUser authenticatedUser;
	  
	  public DataExplorerService(DataExplorerRepository repository,AuthenticatedUser authenticatedUser) {
	        this.repository = repository;
	        this.authenticatedUser = authenticatedUser;    
	  }
	  
	  @Override
	  public Page<DataExplorer> list(Pageable pageable, Specification<DataExplorer> filter) {
		  return repository.findAll(filter, pageable);
	  }
	    
	  @Override
	  public long count(Specification<DataExplorer> filter) {
	        Specification<DataExplorer> spec = (filter != null) ? filter : null;
	        return repository.count(spec);
	  }
	  
	  @Override
	  public void delete(Set <DataExplorer> entity) {
		  if (!authenticatedUser.hasPage(DataExplorerView.class, AccessPageType.DELETED_PAGE)) {
	  ...
	  @Override
	  @Transactional
	  public DataExplorer update(DataExplorer entityValue) {
	      ...
	      if (!authenticatedUser.hasPage(DataExplorerView.class, accessPageType)) {
	    ...
	    public List<DataExplorer> findAll(Specification<DataExplorer> filter){
